...
class CouponControllerTest : DescribeSpec({
    ...
    describe("CouponController") {
        ...
        val controller = CouponController(couponService, buyService)

        describe("buyCoupon") {
            it("should call buyService.buy and return 200 OK") {
                // Given: A product ID
                val productId: Long = 123L

                // When: buyCoupon is called with the productId
                val response: ResponseEntity<String> = controller.buyCoupon(productId)

                // Then: buyService.buy should be called with the productId, and the response should be 200 OK
...
// src/main/kotlin/com/example/coupon/service/BuyService.kt
import com.example.coupon.service.BuyService
import io.kotest.core.spec.style.DescribeSpec
import io.kotest.matchers.shouldBe
